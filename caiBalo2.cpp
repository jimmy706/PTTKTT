...
DoVat * ReadFile(float *W, int *n){
	...
	while(!feof(f)){
		fscanf(f, "%f %f %d %[^\n]s", 
		&dsdv[i].trongLuong, &dsdv[i].giaTri, &dsdv[i].soLuong, dsdv[i].tenDV);		
...
int Chon(float trongLuong, int soLuong, float W){
	if(W > trongLuong){
		if ((int) W / trongLuong <= soLuong) 
...
void Greedy(DoVat *dsdv, int n, float W){
	for(int i = 0; i < n; i++){
		dsdv[i].phuongAn = Chon(dsdv[i].trongLuong, dsdv[i].soLuong, W);
